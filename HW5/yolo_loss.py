...
class YoloLoss(nn.Module):
    ...
    def find_best_iou_boxes(self, box_pred_list, box_target):
        ...
        best_boxes : (tensor) size (-1, 5), containing the boxes which give the best iou among the two (self.B) predictions  ######????(-1,4)
        ...
        traget_bounding_box = box_target[:, :4]
        traget_bounding_box = self.xywh2xyxy(traget_bounding_box) 

        for box_pred in box_pred_list:
            pred_bounding_box = box_pred[:, :4]
            pred_bounding_box = self.xywh2xyxy(pred_bounding_box) 
            ious = torch.diagonal(compute_iou(pred_bounding_box, traget_bounding_box),0)
            ious_list.append(ious)
            
        #print(ious_list)
        best_ious = torch.stack(ious_list, dim=0).max(0)[0]  # 最大 IoU
        best_idx = torch.stack(ious_list, dim=0).max(0)[1]  # 最佳框的索引

        for i,idx in enumerate(list(best_idx)):
            best_box = box_pred_list[idx][i,:]
            best_boxes.append(best_box)

        best_boxes = torch.stack(best_boxes, dim=0)  
    ...
    def get_no_object_loss(self, pred_boxes_list, has_object_map):
        ...
        # Your code here
        loss = 0
        # zero_tensors = torch.zeros(pred_confidence[~has_object_map].size())
        for pred_box in pred_boxes_list:
            pred_confidence = pred_box[:, :, :, 4]
            loss += F.mse_loss(pred_confidence[~has_object_map], torch.zeros(pred_confidence[~has_object_map].size()).to(pred_confidence.device),reduction='sum')
    ...
    def get_regression_loss(self, box_pred_response, box_target_response):
        ...
        ### CODE
        # your code here
        coordinates = box_pred_response[:, :2]
        target_coordinates = box_target_response[:, :2]

        width = box_pred_response[:, 2:]
        target_width = box_target_response[:, 2:]
        reg_loss = torch.sum(torch.pow(coordinates-target_coordinates, 2))
        reg_loss += torch.sum(torch.pow(width - target_width,2))
